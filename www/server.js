...
app.get("/api", (req, res) => {
  res.json({
    ...
    endpoints: {
      "/api/get":
        "returns a list of all objects (students.all and teachers.all)",
      "/api/get/teachers": "returns a list of all teachers (teachers.all)",
      "/get/:index": "returns the person at index)",
      "/get/:key/:content":
        "returns the student that has values in attributes :key that matches value :content",
...
// This is my API that return my information from secret.json file

app.get("/api/my", (req, res) => {
  const secretPath = path.join(__dirname, "../secrets.json");
  if (fs.existsSync(secretPath)) {
    const myinfo = JSON.parse(fs.readFileSync(secretPath));
    myinfo.age = 35;
    myinfo.job = "student";
    res.json(myinfo);
  } else {
